...
@Trainer.register("gradient_descent_acl", constructor="from_partial_objects")
class GradientDescentTrainerACL(Trainer):
    """
    ...
    def __init__(
        ...
        #it should not be optional since trainer is customized for this
        controller: Optional[Controller] = None,
        log_per_updates = 1000,
        ...
        n_task = self.data_loader.batch_sampler.vocab.get_vocab_size('dataset')
        #TODO: find out how to avoid this circular dependency
        self._controller = controller
        # otherwise it reinits it - hacky
        phi = self._controller.phi
        log_filename = f'{serialization_dir}/log.json'
        dataset_names = [key for key in self.data_loader.batch_sampler.vocab.get_token_to_index_vocabulary('dataset')]
        dataset_frequencies = self.data_loader.batch_sampler.vocab._retained_counter['dataset']
        dataset2idx = data_loader.batch_sampler.vocab.get_token_to_index_vocabulary('dataset')
        dataset_sizes = []
        for dataset_name in dataset2idx:
            dataset_sizes.append(dataset_frequencies[dataset_name])
        self.data_loader.batch_sampler.n_task = n_task
        self._controller.__init__(trainer=self, n_task=n_task, phi=phi, log_filename=log_filename, dataset_names=dataset_names, dataset_sizes=dataset_sizes)
    ...
    def _train_epoch(self, epoch: int) -> Dict[str, float]:
        ...
        if self._controller is not None:
            ...
            self._controller.set_lengths(len_data_loader)
            total_len = self._controller.lengths
            self._controller.set_epoch(epoch)

        done_early = False

        i = 0
        for batch_group in batch_group_generator:
            i += 1
            if self._distributed:
            ...
            for batch in batch_group:
                # not sure I get the shape of this batch
                # batch['tokens']['tokens'] ???
                with amp.autocast(self._use_amp):
                    ...
                    #TODO: this is from the original trainer but looks off to me
                    batch_loss += loss.item()
                    #the original trainer does this before rescale gradients
                    if reg_loss is not None:
                        reg_loss = reg_loss / len(batch_group)
                        batch_reg_loss = reg_loss.item()
                        train_reg_loss += batch_reg_loss
                    task_id = batch['dataset'][0]
                    self._controller.insert(task_id, batch, batch_loss)

                #need to print when cur_step is 0 which should be when i = 1?
                if i == 1 or i % self.log_per_updates == 0:
                    remaining_time = str((datetime.datetime.now() - start) / (self._controller.cur_step + 1) * (total_len - self._controller.cur_step - 1)).split('.')[0]
                    logger.info("Epoch {0} Progress {1} / {2}  ({3:.2%})".format(epoch, self._controller.cur_step, total_len, self._controller.cur_step*1.0/total_len))
                    logger.info('Task [{0:2}] updates[{1:6}] train loss[{2:.5f}] remaining[{3}]'.format(task_id, self._controller.updates, train_loss, remaining_time))

                    summary_str = self._controller.summary()
                    for line in summary_str.split("\n"):
                        logger.info(line)

            train_loss += batch_loss
            loss = self._controller.step()

                #This happens in the controller instead
                #if self._scaler is not None:
        ...
        #TODO: try not doing this when there are not enough examples to train from 
        #because then the weights don' have time to get updated
        self._controller.initialization()
        return metrics
    ...
    def from_partial_objects(
        ...
        data_loader: DataLoader,
        controller:Controller = None, #not optional>
        validation_data_loader: DataLoader = None,
        local_rank: int = 0,
        patience: int = None,
        validation_metric: str = "-loss",
        ...
        data_loader.batch_sampler.controller = controller
